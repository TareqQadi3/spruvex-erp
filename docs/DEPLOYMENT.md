...
1. **DNS / TLS**: point your domains (e.g. `app.`, `pos.`, `kds.`, `order.`
   yourdomain.com) at the host(s) running these containers. This repo's
   compose file exposes plain HTTP on host ports (8081-8083, 3000, 3002) —
   put a TLS-terminating reverse proxy or load balancer (Caddy, an existing
   Nginx, your cloud LB) in front for production; it is **not included**
   here since that choice is platform-specific.
