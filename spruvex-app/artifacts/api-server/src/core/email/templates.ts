...
// wrapper/social block is mirrored — same markup — in spruvex-r's
// shared/email/templates.ts, so a customer trying two SpruVex products gets
// one consistent brand). Inline styles only: most email clients strip
// <style> tags in <head>. Arabic/RTL by default.
...
function wrapper(productLabel: string, body: string): string {
  return `
<div dir="rtl" style="font-family:'Segoe UI',Tahoma,Arial,sans-serif;background:#f6f7f4;padding:32px 16px;">
  <div style="max-width:480px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;">
    <div style="background:${BRAND_GREEN};padding:20px 32px;">
      <div style="font-size:20px;font-weight:700;color:#ffffff;">SpruVex <span style="opacity:.85;font-weight:500;">${productLabel}</span></div>
    </div>
...
export function accountCreatedEmail(
  ...
  return {
    subject: "مرحباً بك في SpruVex ERP",
    html: wrapper(
      "ERP",
      `
      <p style="font-size:15px;color:#333;">مرحباً،</p>
      ...
      <div style="margin-top:20px;padding-top:16px;border-top:1px solid #eee;font-size:13px;color:#555;">
        ...
        <ul style="margin:0;padding-inline-start:18px;line-height:1.9;">
          <li>إدارة المبيعات ونقاط البيع</li>
          <li>متابعة المخزون والمستودعات</li>
          <li>إدارة العملاء والموردين</li>
          <li>المحاسبة والفواتير الإلكترونية (ZATCA)</li>
          <li>تقارير وتحليلات الأعمال</li>
        </ul>
      </div>
    `,
    ),
